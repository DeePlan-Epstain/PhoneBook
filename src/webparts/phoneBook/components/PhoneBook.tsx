...
> {
  ...
  FilterDisplayedOptions = () => {
    ...
    if (lowerCaseSearchValue !== "") {
      FilteredContacts = FilteredContacts.filter((contact: IContact) => {
        ...
        return fieldsToSearch.some((field) =>
          field ? field.toLowerCase().includes(lowerCaseSearchValue) : false
        );
      });
    }
    ...
    const CurrPageDisplayOptions = FilteredContacts.slice(
      indexOfFirstOption,
      indexOfLastOption
    );
  ...
  OnSetCurrentPage = async (event: object, page: number) => {
    ...
    const indexOfFirstOption = indexOfLastOption - PageSize;
    const CurrPageDisplayOptions = DisplayOptions.slice(
      indexOfFirstOption,
      indexOfLastOption
    );
        ...
        <div id="phonebook" className={`${styles.PhoneBookBackGroundImage}`}>
          <div className={`${styles.PhoneBookRightContainer}`}>
            <div className={`${styles.PhoneBookTitlesContainer}`}>
              ...
              <img
                className={`${styles.PhoneBookLine20}`}
                src={require("../assets/Line20.svg")}
                alt=""
              />
            </div>
          </div>
                    ...
                    <ThemeProvider theme={theme}>
                      <div className="search-filter-wrapper">
                        <Autocomplete
                          ...
                          popupIcon={
                            <SearchIcon style={{ color: "white[500]" }} />
                          }
